...
class GUI(object):
	def __init__(self):
		...
		#self.mrect = pygame.Rect(10,10,900,560)###&&&
		self.mailmenu = pygame.display.set_mode((920,800))###&&&
		self.MAIL = backend_email2.email(self.mailmenu)###&&&
		...
		emailrect = self.emailpic.get_rect()
		self.therect = [pygame.image.load(os.path.join('EmailDisplay', self.emailurl)), emailrect]
		...
		while True:	#main game loop
			...
			self.background.blit(self.therect[0], self.therect[1])###&&&
	...
	def keysdown(self, thetyped):
		...
		if self.event.type == KEYDOWN and self.event.key == K_RETURN:
			...
			if j == 0:
				...
				#EMAIL identifier	###
				if firstword == 'check':
					self.therect = self.MAIL.display()
					#self.background.blit(therect[0], therect[1])
